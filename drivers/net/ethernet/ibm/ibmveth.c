...
ibmveth_setup_rx_interrupts(struct ibmveth_adapter *adapter)
{
	...
	for (i = 0; i < num; i++) {
		...
		if (rc) {
			netdev_err(netdev,
				   "Failed to request IRQ 0x%x for queue %d, rc=%d\n",
 ...
 *
 * Registers the IRQ handler and enables NAPI for one queue. Used during
 * incremental scale-up when adding new RX queues.
 *
 * Return: 0 on success, negative error code on failure
 */
static int
...
static void ibmveth_free_all_queues(struct ibmveth_adapter *adapter)
{
	...
	/* Clear queue handles only.  queue_irq[] is cleared by
	 * ibmveth_cleanup_rx_interrupts() after free_irq().
	 */
	for (i = 0; i < adapter->num_rx_queues; i++)
 ...
 *
 * Sums per-queue rx_qstats and tx_qstats into the rtnl counters for
 * multi-queue mode. TX errors continue to come from netdev->stats.
 */
