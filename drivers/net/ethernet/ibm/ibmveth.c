...
static struct ibmveth_stat ibmveth_stats[] = {
	...
	{ "h_reg_queue_calls", IBMVETH_STAT_OFF(h_reg_queue_calls) },
	{ "h_reg_lan_calls", IBMVETH_STAT_OFF(h_reg_lan_calls) },
	{ "h_add_buf_queue_calls", IBMVETH_STAT_OFF(h_add_buf_queue_calls) },
	{ "h_add_buf_lan_buffer_calls", IBMVETH_STAT_OFF(h_add_buf_lan_buffer_calls) },
	{ "h_add_buf_lan_buffers_calls", IBMVETH_STAT_OFF(h_add_buf_lan_buffers_calls) },
	{ "h_free_queue_calls", IBMVETH_STAT_OFF(h_free_queue_calls) },
	{ "h_free_lan_calls", IBMVETH_STAT_OFF(h_free_lan_calls) },
	{ "h_send_lan_calls", IBMVETH_STAT_OFF(h_send_lan_calls) },
	{ "h_send_lan_packets", IBMVETH_STAT_OFF(h_send_lan_packets) },
	{ "h_send_lan_busy_retries", IBMVETH_STAT_OFF(h_send_lan_busy_retries) },
	{ "h_send_lan_dropped", IBMVETH_STAT_OFF(h_send_lan_dropped) },
	{ "h_send_lan_failed", IBMVETH_STAT_OFF(h_send_lan_failed) }
};
					   ...
					   unsigned long buff_size)
{
	...
	if (adapter->use_subordinate_queue) {
		...
		adapter->h_add_buf_lan_buffers_calls++;
	}
...
retry:
	...
	adapter->h_reg_lan_calls++;

	if (rc != H_SUCCESS && try_again) {
		do {
			rc = h_free_logical_lan(adapter->vdev->unit_address);
			adapter->h_free_lan_calls++;
			...
			unsigned long desc, unsigned long mss)
{
	...
	do {
		...
		adapter->h_send_lan_calls++;
		if (ret == H_BUSY)
			adapter->h_send_lan_busy_retries++;
	} while ((ret == H_BUSY) && (retry_count--));

	if (ret == H_SUCCESS) {
		adapter->h_send_lan_packets++;
		return 0;
	}

	if (ret == H_DROPPED) {
		adapter->h_send_lan_dropped++;
		netdev_warn_once(adapter->netdev,
				 "tx: hypervisor dropped packet (H_DROPPED)\n");
		return 0;
	}

	adapter->h_send_lan_failed++;
	netdev_err(adapter->netdev,
