...
{
	struct ibmveth_rx_queue_stats *qstats = &adapter->rx_qstats[queue_index];
				     ...
				     int new_count, int rxq_entries)
{
	...
	if (new_count > old_count) {
		...
		/*
		 * Survivors were never in the teardown loops above, which
		 * walk new_count..old_count-1 only, so the shrink itself
		 * does not stop them. Restart is defensive: a queue left
		 * masked with NAPI idle by an unrelated path (the poll tail
		 * exits masked when enable_irq() fails) has nothing else to
		 * revive it, and replenish plus one poll is cheap here.
		 */
