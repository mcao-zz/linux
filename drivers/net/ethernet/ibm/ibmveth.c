					    ...
					    int queue_index)
{
	...
	if (adapter->multi_queue) {
		...
		atomic64_inc(&adapter->hcall_stats.add_bufs_queue);
	} else if (filled == 1) {
		rc = h_add_logical_lan_buffer(vdev->unit_address,
					      descs[0].desc);
		atomic64_inc(&adapter->hcall_stats.add_buf);
	} else {
		...
		atomic64_inc(&adapter->hcall_stats.add_bufs);
	}
					 ...
					 struct ibmveth_replenish_fail *fail)
{
	...
	while (remaining > 0) {
		...
		atomic64_add(filled, &adapter->replenish_add_buff_success);
		remaining -= filled;
...
hcall_failure:
		...
		atomic64_add(filled, &adapter->replenish_add_buff_failure);
				   ...
				   int queue_index)
{
	...
	atomic64_inc(&adapter->replenish_task_cycles);
...
retry:
	...
	atomic64_inc(&adapter->hcall_stats.reg_lan);

	if (rc != H_SUCCESS && try_again) {
		do {
			rc = h_free_logical_lan(adapter->vdev->unit_address);
			atomic64_inc(&adapter->hcall_stats.free_lan);
				   ...
				   int queue_index)
{
	...
	atomic64_inc(&adapter->hcall_stats.reg_lan_queue);
		...
		if (!virq) {
			...
			do {
				free_rc = h_free_logical_lan_queue(ua, handle);
				  ...
				  (free_rc == H_BUSY));
			atomic64_inc(&adapter->hcall_stats.free_lan_queue);
				     ...
				     int new_count, int rxq_entries)
{
	...
	if (new_count > old_count) {
		 ...
		 * loop. Scale-down (ethtool -L shrinking RX) can leave Q0
		 * masked with NAPI idle and nothing pending; kick would
		 * no-op and RX stays down. Restart those survivors.
		 */
					...
					int index)
{
	...
	if (offset == IBMVETH_STAT_OFF(replenish_task_cycles))
		return atomic64_read(&adapter->replenish_task_cycles);
	if (offset == IBMVETH_STAT_OFF(replenish_no_mem))
		return atomic64_read(&adapter->replenish_no_mem);
	if (offset == IBMVETH_STAT_OFF(replenish_add_buff_failure))
		return atomic64_read(&adapter->replenish_add_buff_failure);
	if (offset == IBMVETH_STAT_OFF(replenish_add_buff_success))
		return atomic64_read(&adapter->replenish_add_buff_success);
	...
	if (offset == IBMVETH_STAT_OFF(hcall_stats.reg_lan_queue))
		return atomic64_read(&adapter->hcall_stats.reg_lan_queue);
	if (offset == IBMVETH_STAT_OFF(hcall_stats.reg_lan))
		return atomic64_read(&adapter->hcall_stats.reg_lan);
	if (offset == IBMVETH_STAT_OFF(hcall_stats.add_bufs_queue))
		return atomic64_read(&adapter->hcall_stats.add_bufs_queue);
	if (offset == IBMVETH_STAT_OFF(hcall_stats.add_bufs))
		return atomic64_read(&adapter->hcall_stats.add_bufs);
	if (offset == IBMVETH_STAT_OFF(hcall_stats.add_buf))
		return atomic64_read(&adapter->hcall_stats.add_buf);
	if (offset == IBMVETH_STAT_OFF(hcall_stats.free_lan_queue))
		return atomic64_read(&adapter->hcall_stats.free_lan_queue);
	if (offset == IBMVETH_STAT_OFF(hcall_stats.free_lan))
		return atomic64_read(&adapter->hcall_stats.free_lan);
	if (offset == IBMVETH_STAT_OFF(hcall_stats.send_lan))
		return atomic64_read(&adapter->hcall_stats.send_lan);
			...
			unsigned long desc, unsigned long mss)
{
	...
	atomic64_inc(&adapter->hcall_stats.send_lan);
	return 0;
}
