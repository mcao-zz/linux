...
static int ibmveth_disable_irq(struct ibmveth_adapter *adapter, int queue_index)
{
	...
	if (adapter->num_rx_queues == 1 || queue_index == 0) {
		...
		if (rc == H_PARAMETER) {
			netdev_err(adapter->netdev,
				   "H_PARAMETER from H_DISABLE_VIO_INTERRUPT: queue=%d virq=%lu hwirq=%lu unit_address=0x%x\n",
...
static int ibmveth_enable_irq(struct ibmveth_adapter *adapter, int queue_index)
{
	...
	if (adapter->num_rx_queues == 1 || queue_index == 0) {
		...
		if (rc == H_PARAMETER) {
			netdev_err(adapter->netdev,
				   "H_PARAMETER from H_ENABLE_VIO_INTERRUPT: queue=%d virq=%lu hwirq=%lu unit_address=0x%x\n",
				   queue_index, irq, hwirq, adapter->vdev->unit_address);
...
static int ibmveth_resize_rx_queues_incremental(struct ibmveth_adapter *adapter,
						int new_count, int rxq_entries)
{
	...
	netdev_info(netdev, "Incrementally resizing RX queues: %d → %d\n",
		    old_count, new_count);

	if (new_count > old_count) {
		/* Scale-up: Add new queues [old_count, new_count) */
		...
		for (i = old_count; i < new_count; i++) {
			...
			if (rc) {
				netdev_err(netdev, "Failed to setup IRQ for queue %d: %d\n",
					   i, rc);
		...
		/* Scale-down: Remove excess queues [new_count, old_count) */
		netdev_dbg(netdev, "Scale-down: removing queues %d-%d\n",
			   new_count, old_count - 1);

		/* Disable NAPI for excess queues */
...
cleanup_new_queues:
	/* Cleanup any queues we successfully added before the failure */
	failed_queue = i;
	...
	/* Keep old queue count - existing queues still work */
				      ...
				      u64 mac_address)
{
	...
	for (i = 1; i < adapter->num_rx_queues; i++) {
		rc = ibmveth_register_single_rx_queue(adapter, i, mac_address);
		if (rc) {
			/* Validate that queue was properly registered */
