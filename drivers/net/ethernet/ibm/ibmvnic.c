				...
				struct ibmvnic_long_term_buff *ltb, int size)
{
	...
	struct ibmvnic_long_term_buff new_ltb = {0};
	int rc;
	...
	if (!reuse_ltb(ltb, size)) {
		...
		/* Allocate before releasing, so that a failure here leaves
		 * the caller with the buffer it already had rather than with
		 * none at all.
		 */
	...
	/* Take the generation before asking, so that a connection lost while
	 * we wait below leaves the buffer looking like what it now is: not
	 * mapped anywhere the VIOS still knows about.
	 */
				...
				struct ibmvnic_long_term_buff *ltb)
{
	...
	/* The VIOS drops every mapping when the crq connection is
	 * re-established, so a buffer mapped on an earlier connection is
	 * already unmapped at the remote end.
	 *
	 * It also unmaps them for the following resets:
 ...
 * Overshooting IBMVNIC_MAX_LTB_SIZE fails the allocation outright.
 */
static u64 rx_pool_entries(struct ibmvnic_adapter *adapter)
{
	...
	return min_t(u64, adapter->req_rx_add_entries_per_subcrq,
		     IBMVNIC_MAX_LTB_SIZE / buff_size);
}
				   ...
				   struct ibmvnic_adapter *adapter)
{
	...
	case PARTIALSUCCESS:
		...
		/* The backing device runs at one of a small set of fixed
		 * sizes, so the vnicserver answers every in-range REQ_MTU
		 * with PARTIALSUCCESS and the size it settled on. Where that
...
/**
 * ibmvnic_crq_deactivate() - Note that the crq connection is gone.
 * @crq: crq queue that is no longer connected
 *
 * The VIOS forgets every long term buffer mapping when the connection is
 * dropped, so bump the generation. Buffers mapped on an earlier one can
 * then be told apart and released without asking the VIOS to unmap them
 * a second time.
 */
